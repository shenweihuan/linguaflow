...
**30 秒证据：** 注册 → 选英语A2 → 进单词闪卡 → 翻第一张牌（"abandon"）→ 听发音 → 自己读→ 系统评分 → 10分钟后你已经学了15个词。没有枯燥的背单词表，只有像翻牌一样的流畅体验。
...
> **"你不需要体验全部 9 个功能模块才能学好外语。先用这 3 个，形成最小学习闭环。"** — 芒格

| 优先级 | 功能 | 一句话 | 为什么 |
|--------|------|--------|--------|
| ★ **单词闪卡** | 3D翻转记忆 | "我要背单词" | 词汇是一切的基础 |
| ★ **语法练习** | 即时反馈训练 | "我要练语法" | 从错误中学习最快 |
| ★ **进度追踪** | Bento Grid仪表盘 | "我学到哪了" | 可视化驱动持续动力 |

> **第一次用？注册登录 → 选一门课 → 打开单词闪卡 → 开始翻牌。30秒内出结果。**
